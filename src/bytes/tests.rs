...
fn test_new() {
    let bump = Bump::new();
    let bytes = Bytes::new(bump_vec![in &bump; 1, 2, 3]);
    drop(bytes);
}
...
fn test_set_raw() {
    let bump = Bump::new();
    let mut bytes = Bytes::new(bump_vec![in &bump; 1, 2, 3]);
...
fn test_prepend() {
    let bump = Bump::new();
    let mut bytes = Bytes::new(bump_vec![in &bump; 4,5,6]);
    bytes.prepend(&[1, 2, 3]);
    assert_eq!(bytes.as_raw(), &bump_vec![in &bump; 1, 2, 3, 4, 5, 6]);
    drop(bytes);
}
