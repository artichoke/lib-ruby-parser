...
#[repr(C)]
pub struct {{ helper node-rust-camelcase-name }}{{ helper node-generic-lifetime }} {
{{ each node-field }}<dnl>
...
impl{{ helper node-generic-lifetime }} {{ helper node-rust-camelcase-name }}{{ helper node-generic-lifetime }} {
    // getters
{{ each node-field }}
    /// Returns `{{ helper node-field-rust-field-name }}` field
    pub fn get_{{ helper node-field-name }}(&self) -> {{ helper node-field-getter-return-type }} {
...
{{ end }}
    ...
    #[allow(dead_code)]
    pub(crate) fn into_internal<'b>(self, bump: &'b bumpalo::Bump) -> &'b super::Internal{{ helper node-rust-camelcase-name }}{{ helper node-generic-lifetime }} {
        let Self { {{ each node-field }}{{ helper node-field-rust-field-name }}, {{ end }} } = self;
        bump.alloc(super::Internal{{ helper node-rust-camelcase-name }} { {{ each node-field }}{{ helper node-field-rust-field-name }}, {{ end }} })
    }
}

impl<'a> InnerNode<'a> for {{ helper node-rust-camelcase-name }}{{ if node-has-ref-fields }}<'a>{{ else }}{{ end }} {
    fn expression(&'a self) -> &Loc {
        &self.expression_l
    }

    #[allow(unused_qualifications)]
    fn inspected_children(&'a self, indent: usize) -> std::vec::Vec<std::string::String> {
...
pub(crate) fn codegen(node: &lib_ruby_parser_nodes::Node) {
    ...
    fns.register_helper(
        "node-field-getter-return-type",
        local_helpers::node_field_getter_return_type,
    );
...
mod local_helpers {
    ...
    pub(crate) fn imports(node: &Node) -> String {
        ...
        {
            imports.push("crate::use_native_or_external!(String);");
        }
    ...
    pub(crate) fn node_field_getter_return_type(node_with_field: &NodeWithField) -> String {
        use lib_ruby_parser_nodes::NodeFieldType;
        match node_with_field.field.field_type {
